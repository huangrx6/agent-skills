...
取用"的稳定工作流。每节标落地状态：【✅ 已实现】【约定=规则在、机制未接】。
...
| Style | 构图、排版性格、色彩语法、图片/图形/动画语言 | 公司身份、官方 Logo、品牌锁定色 | `styles/<name>/` |
