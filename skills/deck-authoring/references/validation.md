...
- **字段集不是闭合的**：写了渲染器不认得的键（`fontSize` / `x` / `y` …）不会报错，
  它们只是不起作用。唯一被主动拦下的是 `color`（只接受 `"overprint"`）—— 见
  `style-architecture.md` 的 schema 一节。
- **② 文字溢出是估算**：按字宽表算，不是浏览器的真实折行 —— 它只挡“明显放不下”，
  边界附近的漏报与误报都在所难免。精确断行只有浏览器知道。
- **审美不承诺**：配色好不好看、版式舒不舒服，这六项一律不管（`check.py` 的
  docstring 第一句就是这个意思）。
