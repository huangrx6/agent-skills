...
台上的人需要四样东西，都在同一屏内：**现在第几页 / 下一页是什么 / 这一页要说什么 /
讲了多久**。按 `S` 开/关（也可以 `?present` 直接讲）：

| 键 | 做什么 |
| --- | --- |
| `S` | 开/关讲稿层（开的一瞬间开始计时） |
| `B` | 黑屏 —— 只盖幻灯片，不盖讲稿（对着观众黑，对着自己还能看） |
| `R` | 计时归零 |
| `← / →` | 翻页（讲稿跟着走；底部的 `下一页 · 标题 · 版式` 同步） |

讲稿在 spec 里逐页写 `notes`：

```jsonc
{ "type": "content-image", "title": "关键路径", "image": "...",
  "notes": "这一页停 40 秒：先说结论（延迟降了 62%），再解释为什么不是加机器。" }
```

它是**给人读的文本**，不参与排版：不发任何元素（所以不进清单、不进测量），
只随产物走一份 JSON 载荷，由讲稿层读。产物仍是一个自包含单文件 ——
不需要服务器、不需要第二个文件（现场演出时少一个依赖就少一处出错）。

**它盖住下半页，所以它是给你自己看的**：投屏时不要开 `S`（观众会看到你的讲稿）。
单窗口是刻意的取舍：第二窗口那套需要跨窗口同步与断开恢复状态机 ——
实测最容易在现场出问题的正是它。要把讲稿放在另一块屏上，现在得靠系统级镜像，
而镜像会把讲稿一起显给观众 —— 这种场合不用讲稿层，把要点写进幻灯片本身。

**合同门**：产物里的锚点（九个 id + `data-slide` + 窄接口 `window.__deck_ui`）
由 `check.py` 当**阻塞问题**验 —— 缺一个就是“拼产物时落下了东西”，
而且讲稿层找不到元素时会静默 `return`（现场才发现按 `S` 没反应，太晚）。
...
**先问 HTML**：它是**唯一带演示态**的形态（`?present` 一页一屏、`←/→`、`S` 讲稿层、`F`）且自包含
