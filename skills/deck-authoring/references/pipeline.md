...
"全局怎么串起来"；各模块细节在对应规则文件（§55 的映射表）。规范里**规则在、
本仓库未接机**的机制，在节标题上用【约定】标出。
...
10_shape_icon=约定；11_table=约定；12_motion=animation；13_slide_dsl=
...
Repair 保证质量，Export QA 保证最终交付。本仓库**已接机的范围**就是本文各节：
标【约定】的是只保留规则、还没接机的部分。
