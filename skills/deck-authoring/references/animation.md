...
+ **确定性地基** + **导出**。规范里**规则在、效果未接**的机制在节标题上标【约定】。
...
blurToClear accentSweep。**本仓库提供 4 个 preset**：fadeRise、maskRevealY、
imageReveal、growX；另有编外 pop（散点）。**growY（柱）与 pathDraw（折线）不适用**（图表由 G2 静态渲染）；
countUp/highlight/sharedMove 等**没有实现**。高级 Shader 效果不进 preset（见 §5）。
