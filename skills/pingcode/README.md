...
```sh
cd skills/pingcode
python3 -m unittest discover -s tests -v     # 120 条：全绿
...
CLI 层证明了 dry-run 不发请求、写操作打到正确端点且 body 里是解析后的真 id。

## 已知限制与未验证项

| 项 | 状态 |
| --- | --- |
| 用真实租户跑通只读 + 写全链路 | **未实测** —— 代码与测试都在离线替身上验证过，真实凭据下的行为待验 |
| `assignee_id` 是否接受 `me` 这类占位 | 未实测；本 skill 一律解析成真实 id，不依赖服务端支持 |
| `state.type` 的完整取值集合 | 官方示例只给了 `pending`；判据取「不等于 `completed` 即未完成」（宁可多列） |
| 令牌响应里 `expires_in` 是绝对时间戳还是秒数 | 官方示例像绝对时间戳；代码两种都接，`auth status` 会打出算出来的到期时间 |
| 状态流转的完整规则 | 只查「该类型可用状态」；更深的状态机校验交给服务端，报错原样翻出来 |
| `POST /v1/pjm/workitems/search`（复杂过滤） | 未封装，走 `api` 逃生口 |
| 批量改（`PATCH /v1/pjm/workitems`） | 未封装成子命令（官方只支持单属性 + 单值 + ≤100 个 id），走逃生口 |
| 附件 / 评论列表 / 关注人 / 关联 / 测试管理 / 需求 / 工单 | 未封装，走 `api` 逃生口 |
