...
有逃生阀，但它只允许**仓库之外**的位置，两种落法会被拒：

| 目标 | 为什么拒 |
| --- | --- |
| 落在本仓库的工作树里（`<仓库>/sub`） | `git status` 会把它当成未跟踪内容收进来、`clean -fd` 也会去删它 —— 正是「放同级」要避开的事 |
| 落在 `.git` 里 | 那是 git 自己放东西的地方 |
| 落在**另一个**仓库里 | 那个仓库会把新目录当成未跟踪内容收进去 |

建好之后的报告会说清这是**默认位置**还是**你指定的位置**（默认位置也好、逃生阀也好，
反正都在仓库之外 —— 所以「不用改 .gitignore」那句话是真的）。

> 这一条的来历：最早只检查了「落在**别的**仓库里」，同仓库那一支被
> `not same_path(...)` 漏掉了 —— `--at <本仓库>/sub` 会被放行，输出还写着
> 「这个目录在仓库之外」。修复时又踩了一次符号链接（git 给 `/private/var/...`，
> 传进来的可能是 `/var/...`，前缀比较对不上）。两处都有用例守着。
