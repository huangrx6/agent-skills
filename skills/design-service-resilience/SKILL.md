...
description: "设计、评审和完善生产级服务韧性方案，包括端到端超时预算、重试责任、指数退避与抖动、重试预算、熔断、舱壁隔离、限流、负载削减、背压、队列容量、降级、缓存兜底、异步任务恢复、健康检查和故障注入。用于微服务、分布式系统、异步任务、高并发服务。错误分类与异常传播、架构边界、容量模型参考独立规范。"
---
