...
description: "把 UI 页面截图、数据大屏、设计稿图片转成可直接交付前端和 AI Coding Agent 的页面布局规格。视觉语义（Agent 原生视觉或远端 VLM：VISUAL_REMOTE=true 强制远端，否则优先原生，不绑定特定 Agent、零配置）+ image_probe.py 像素级确定性测量双轨证据，提取区域、组件、文案、状态、栅格、尺寸、间距、颜色、字号、圆角、边框、阴影、图表和表格，保留证据与置信度。本 Skill 不负责通用图片语义理解（文字提取、对象关系、架构图节点）。"
---
