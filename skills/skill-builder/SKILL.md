...
**工具报“成功”不等于文件真的变了。** 三次实测：编辑工具回“成功替换 1 块”而文件其实没动，
我却把“已加入 checklist”写进了报告（`git log -S"check_leakage.py" -- skills/skill-builder/SKILL.md` 无输出）；
“SKILL.md（142 行）”实际 74；**已经跑了 preflight、快照写着 146，我仍写成 137**。
共同点：**数字与存在性来自记忆，而不是测量**。前置步骤：写报告前跑
`python3 scripts/preflight.py`，每个数字从它的输出里抄。

> ⚠ **这条防线没有闭合。** 报告是自由文本，没有任何东西会验证它 —— 第三次正是
> “跑了工具但仍抄错”。所以它**目前仍然依赖“记得照抄”**，与它想防的错误同属一个薄弱环节。
> 真闭合需要报告文本直接从快照生成，那在聊天报告里做不到。**别把“加了 preflight”当成已堵住。**

**唯一还能做的缓解（刻意为之，不是习惯性贴日志）**：报告里**把 preflight 的原始输出整块贴在
数字旁边**，而不只是写数字。散文仍然没有被验证 —— 但“信任散文”变成了“散文与相邻的原始数据
一起摆出来，任何人（包括未来的你）一眼就能发现不一致”。**所以报告末尾那段不要省。**

顺带：preflight 会查**孤儿脚本** —— `scripts/` 里有文件、检查在跑，但从 SKILL.md 和 references 里
一个字都找不到它。脚本存在但没人叫你去跑，等于没接线（实测：第一次跑就报出 `validate_spec.py`）。
