...
> **当前状态（2026-08-18 探查）**：`01 Projects/` 目录**为空**——所有 5 个项目目录已被重置或迁移。Agent 第一次处理 Projects 相关操作前，必须先 `ls` 真实目录，不能依赖本节列出的目录列表。

历史项目（已被移除，仅供参考）：

- `01 Projects/01 某历史项目/`
- `01 Projects/02 内部系统B/`
- `01 Projects/03 某平台/`
- `01 Projects/04 某审查系统/`
- `01 Projects/05 某开源框架/`

**新增项目时的约定**：项目目录使用数字前缀方便扫描（`NN 项目名/`）。需要持续跟踪的活跃项目，应有一篇 `项目 - 名称` 主页。会议记录、实现记录、交付记录只有在直接服务该项目结果时才放入对应项目目录。

**索引入口**：`01 Projects/项目区 - 索引.md`（如果不存在，agent 第一次操作 Projects 时应建议用户创建）。

## 当前 Areas 结构

> **当前状态（2026-08-18 探查）**：`02 Areas/` 目录**为空**——所有 2 个领域目录已被重置或迁移。Agent 第一次处理 Areas 相关操作前，必须先 `ls` 真实目录，不能依赖本节列出的目录列表。
