...
1. 命令行 `--scheme classic`（显式最优先）
2. 规格里的 `mood` 那句原话 —— `专业 / 商务 / 企业` → classic，`工程 / 代码 / 文档 / 等宽` → engineering，
   `黑白 / 打印 / 单色` → print，`深色 / dark` → night，`蓝图 / 蓝底` → blueprint
3. 规格里已有的 `visual` 方向：`night` 有明确对应物就照搬，其余用默认

**未知方案名判失败**（`KeyError` + 列出可用值），不 fallback —— 和 `kind` 一条规矩。
