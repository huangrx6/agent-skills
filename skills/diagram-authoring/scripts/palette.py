...
# **默认的边 kind**：不写 `kind` 就是它。有两个地方要用这个数 —— 画线型
# （`emit.arrow_element`）与“诊断档要不要自动补标签”（`emit.edge_kind_label`）。
# 各写一个字面量的话，以后改默认值会出现“线型按新的画、标签按旧的补”这种静默分裂。
DEFAULT_EDGE_KIND = "sync"
