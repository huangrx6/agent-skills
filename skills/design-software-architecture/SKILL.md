...
description: "设计、评审和演进软件系统架构，包括架构约束与质量属性、模块和服务边界、模块化单体与微服务选择、分层/六边形/事件驱动/CQRS 等架构风格、同步与异步集成、数据所有权与一致性、可用性/扩展性/性能/安全/可运维性、部署拓扑、故障域、遗留系统现代化、ADR、C4 架构图、架构风险和评审门禁。用于新系统架构设计、现有系统重构、单体拆分、微服务边界评审、跨团队系统集成、重大技术选型、架构文档治理。API 契约细节、数据库表结构、异常处理、日志格式与代码级书写规范参考独立规范。"
---
