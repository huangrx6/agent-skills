...
```text
已完成：回填 6 份 README ・ 清死文件 ・ pingcode 建项目实测
         ↓
1. 补 2 个 evals              ← 无前置，纯开发
2. references 瘦身            ← 不主动做：等下一次真需求逼出来（硬塞只会把三个 skill 一起顶到 150 行）
3. pingcode 的剩余未实测项   ← 需要你确认（会在租户里留东西）
4. 触发日志                   ← 前置：确认可挂载的观察点
5. 其余候选                   ← 前置：第 4 步的数据
```

第 4 件才是真正改变后续所有判断的那一件 —— 在它到位之前，「该不该建新 skill」只能靠印象。
