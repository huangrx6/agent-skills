...
```text
已完成：回填 6 份 README ・ 清死文件 ・ 补 2 个 evals ・ 触发日志 ・ 重看 description
         ・ 体检接进 pre-commit ・ pingcode 建项目实测 + 镜像 32 条工作项
         ↓
1. 给 WLRR 补机械部分 or 写明不做   ← 无前置，就剩下它一个悬着
2. 其余候选（pingcode-plan 等）     ← 前置：日志里出现真实需求
3. references 瘦身                  ← 不主动做；但 skill-builder 已只剩 2 行余量，
                                      下一次往正文加规则时会先被迫瘦身
```
